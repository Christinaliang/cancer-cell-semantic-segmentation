...
for idx, (train_indices, test_indices) in enumerate(cv_indices):
    ...
    print('Split {} Trainset size: {}. Number of mini-batch: {}'.format(idx, len(trainset), math.ceil(len(trainset)/batch_size)))
    testset = CellImages(data_dir, test_indices, img_transform=transform)
    print('Split {} Testset size: {}. Number of mini-batch: {}'.format(idx, len(testset), math.ceil(len(testset)/batch_size)))
    ...
    for lr in lrs:
        ...
        for epoch in range(start_epoch, start_epoch+10):
            ...
            test_results = test(epoch, device, testloader, net, criterion, image_size, best_acc, is_save=False, is_print_mb=False)
